...
describe("Supabase Credit Ledger repository", () => {
  it("uses privileged RPC functions for initialization and mutations", async () => {
    const rpc = vi.fn(async (name: string) => ({
      data:
        name === "claim_credit_ledger_free_trial"
          ? true
          : {
    ...
    await Effect.runPromise(repository.claimFreeTrialForLearner("learner-1"));
    ...
    expect(rpc.mock.calls).toEqual([
      ...
      ["claim_credit_ledger_free_trial", { p_learner_id: "learner-1" }],
